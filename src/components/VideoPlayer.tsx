...
export function VideoPlayer({ channel, onNextChannel, onPrevChannel, onBackMobile }: VideoPlayerProps) {
  ...
  // Load channel avec retry intelligent — tous les flux passent par notre proxy anti-blocage
  const loadStream = useCallback(async (attempt = 0) => {
    ...
    try {
      if (isHls && Hls.isSupported()) {
        ...
        // Préchargement proactif du prochain segment pour limiter les coupures
        hls.on(Hls.Events.FRAG_LOADED, (_event, data) => {
          const frag = data.frag;
          if (frag && frag.sn !== undefined && typeof frag.sn === "number") {
            const nextSn = frag.sn + 1;
            const level = hls.levels[hls.currentLevel];
            const nextFrag = level?.details?.fragments?.find((f) => f.sn === nextSn);
            if (nextFrag?.url) prefetchSegment(nextFrag.url);
          }
        });
  ...
  const handleRetry = useCallback(() => { loadStream(0); }, [loadStream]);
  ...
  }, [togglePlay, toggleFullscreen, togglePip, toggleMute, cycleAspectRatio, handleVolumeChange, volume, onNextChannel, onPrevChannel, onBackMobile, handleRetry]);
            ...
            <div className="text-center max-w-md slide-in-up">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <button onClick={(e) => { e.stopPropagation(); handleRetry(); }}
                  className="focusable w-full sm:w-auto flex items-center justify-center gap-2 bg-accent-500 hover:bg-accent-600 text-white px-6 py-3 sm:py-2.5 rounded-xl transition-colors font-medium text-sm">
                ...
                {onNextChannel && (
                  <button onClick={(e) => { e.stopPropagation(); onNextChannel(); }}
                    className="focusable w-full sm:w-auto bg-dark-700 hover:bg-dark-600 text-white px-4 py-3 sm:py-2.5 rounded-xl transition-colors font-medium text-sm">
                    Chaîne suivante
                  </button>
                )}
              </div>
