...
import { proxyStreamUrl } from "@/lib/proxy";

interface VideoPlayerProps {
  channel: Channel;
  onNextChannel?: () => void;
  onPrevChannel?: () => void;
  /** Sur mobile/tablette : revient à la liste des chaînes sans quitter la playlist */
  onBackMobile?: () => void;
}
...
export function VideoPlayer({ channel, onNextChannel, onPrevChannel, onBackMobile }: VideoPlayerProps) {
  ...
  // Load channel — all streams go through our bypass proxy
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;

    setError(null); setLoading(true); setIsPlaying(false);
    setQualities([]); setCurrentQuality(-1); setShowQualityMenu(false);

    if (hlsRef.current) { hlsRef.current.destroy(); hlsRef.current = null; }

    const rawUrl = channel.url;
    const proxiedUrl = proxyStreamUrl(rawUrl);
    const isHls = rawUrl.includes(".m3u8") || rawUrl.includes("/live/") || rawUrl.includes("type=m3u_plus") || rawUrl.includes("/play/") || rawUrl.includes("m3u8");
    ...
    if (isHls && Hls.isSupported()) {
      ...
      hlsRef.current = hls;
      // HLS through proxy — the proxy rewrites M3U8 so all sub-URLs also go through proxy
    ...
    const onError = () => { setError("Impossible de lire ce flux"); setLoading(false); };
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        // Bouton "retour" des télécommandes Android TV / boxes : selon le navigateur ça
        // arrive comme "GoBack", "BrowserBack" ou parfois "Backspace".
        case "GoBack":
        case "BrowserBack":
          if (!document.fullscreenElement && onBackMobile) { e.preventDefault(); onBackMobile(); }
          break;
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [togglePlay, toggleFullscreen, togglePip, toggleMute, cycleAspectRatio, handleVolumeChange, volume, onNextChannel, onPrevChannel, onBackMobile]);
  ...
  return (
    <div ref={containerRef} className="flex-1 flex flex-col bg-black relative"
      ...
      {/* Retour vers la liste — mobile/tablette uniquement (sur bureau/TV on utilise le header) */}
      {onBackMobile && (
        <button
          onClick={(e) => { e.stopPropagation(); onBackMobile(); }}
          className={`md:hidden focusable absolute top-3 left-3 z-20 flex items-center justify-center w-10 h-10 rounded-full bg-black/50 backdrop-blur-sm text-white transition-opacity ${showControls ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      ...
      <div className="flex-1 relative flex items-center justify-center" onClick={() => { togglePlay(); resetHideTimer(); }}>
        ...
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/70">
            <div className="text-center slide-in-up">
              ...
              <p className="text-gray-600 text-xs mt-1">Connexion au flux...</p>
            </div>
          </div>
        )}
        ...
        {error && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/85">
            <div className="text-center max-w-md slide-in-up">
              ...
              <p className="text-gray-500 text-sm mb-5">Le flux n&apos;est peut-être plus disponible.</p>
              <button onClick={(e) => {
                e.stopPropagation(); setError(null); setLoading(true);
                if (hlsRef.current) { hlsRef.current.destroy(); hlsRef.current = null; }
                const video = videoRef.current;
                if (video) { video.src = proxyStreamUrl(channel.url); video.load(); }
              }} className="bg-accent-500 hover:bg-accent-600 text-white px-6 py-2.5 rounded-xl transition-colors font-medium text-sm">
                Réessayer
              </button>
            </div>
          </div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 transition-all duration-300 ${showControls ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2 pointer-events-none"}`}>
        <div className="bg-gradient-to-t from-black via-black/80 to-transparent pt-10 sm:pt-16 pb-3 px-2.5 sm:px-4">
          <div className="flex items-center justify-between mb-2 sm:mb-2.5 gap-2 min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
              ...
              <span className="hidden sm:inline text-gray-500 text-xs shrink-0">{channel.group}</span>
            </div>
            <div className="hidden sm:flex items-center gap-2 text-[10px] text-gray-500 shrink-0">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 overflow-x-auto no-scrollbar">
            {onPrevChannel && (
              <button onClick={(e) => { e.stopPropagation(); onPrevChannel(); }} className="focusable shrink-0 p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5" title="Chaîne précédente (Shift+↑)">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              </button>
            )}

            <button onClick={(e) => { e.stopPropagation(); togglePlay(); }} className="focusable shrink-0 p-3 sm:p-2 text-white hover:text-accent-300 transition-colors rounded-lg hover:bg-white/5" title="Lecture/Pause">
              {isPlaying ? (
            ...
            {onNextChannel && (
              <button onClick={(e) => { e.stopPropagation(); onNextChannel(); }} className="focusable shrink-0 p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5" title="Chaîne suivante (Shift+↓)">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
            )}

            <div className="hidden sm:block w-px h-4 bg-gray-800 mx-1 shrink-0" />

            <button onClick={(e) => { e.stopPropagation(); toggleMute(); }} className="focusable shrink-0 p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5" title="Muet (M)">
              {isMuted || volume === 0 ? (
            ...
            <input type="range" min="0" max="1" step="0.05" value={isMuted ? 0 : volume}
              ...
              onClick={(e) => e.stopPropagation()} className="hidden sm:block w-20 shrink-0" />
            ...
            {qualities.length > 0 && (
              <div className="relative shrink-0">
                <button onClick={(e) => { e.stopPropagation(); setShowQualityMenu(!showQualityMenu); }}
                  className="focusable p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5 text-xs font-medium" title="Qualité">
                ...
                {showQualityMenu && (
                  <div className="absolute bottom-full right-0 mb-2 bg-dark-800 border border-dark-500 rounded-xl overflow-hidden shadow-2xl min-w-[140px]" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => setQuality(-1)}
                      className={`focusable w-full text-left px-4 py-2.5 sm:py-2 text-xs hover:bg-dark-600 transition-colors ${currentQuality === -1 ? "text-accent-300 bg-accent-500/10" : "text-gray-300"}`}>Auto</button>
                    {qualities.sort((a, b) => b.height - a.height).map((q) => (
                      <button key={q.index} onClick={() => setQuality(q.index)}
                        className={`focusable w-full text-left px-4 py-2.5 sm:py-2 text-xs hover:bg-dark-600 transition-colors ${currentQuality === q.index ? "text-accent-300 bg-accent-500/10" : "text-gray-300"}`}>
            ...
            <button onClick={(e) => { e.stopPropagation(); cycleAspectRatio(); }}
              className="focusable shrink-0 p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5" title={`Ratio: ${aspectRatio} (A)`}>
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg>
            </button>

            <button onClick={(e) => { e.stopPropagation(); togglePip(); }}
              className={`focusable shrink-0 p-3 sm:p-2 transition-colors rounded-lg hover:bg-white/5 ${isPip ? "text-accent-300" : "text-gray-400 hover:text-white"}`} title="Picture-in-Picture (P)">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            </button>

            <button onClick={(e) => { e.stopPropagation(); toggleFullscreen(); }}
              className="focusable shrink-0 p-3 sm:p-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5" title="Plein écran (F)">
              {isFullscreen ? (
        ...
        <div className="absolute inset-0 bg-black/85 flex items-center justify-center z-50" onClick={() => setShowShortcuts(false)}>
          <div className="glass border border-dark-500 rounded-2xl p-6 max-w-sm w-full slide-in-up" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm">
              {[["Espace / K","Lecture / Pause"],["F","Plein écran"],["P","Picture-in-Picture"],["M","Muet / Son"],["A","Changer le ratio"],["↑ / ↓","Volume +/-"],["Shift + ↑","Chaîne précédente"],["Shift + ↓","Chaîne suivante"],["⌘K","Rechercher"],["?","Afficher raccourcis"]].map(([key, desc]) => (
